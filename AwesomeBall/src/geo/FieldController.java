...
public class FieldController extends FieldModel {
	...
	public void setSize(double x, double y, double width, double height) {
		...
		// 1) ligne verticale
		this.getSides().add(
				new Line2D.Double(this.getMaxX(), goalY, this.getMaxX(),
						goalMaxY));

		// 2) ligne du haut
		this.getSides().add(
				new Line2D.Double(this.getMaxX(), this.getY(), this.getMaxX(),
						goalY));

		// 3) ligne du bas
		this.getSides().add(
				new Line2D.Double(getMaxX(), goalMaxY, this.getMaxX(), this
						.getMaxY()));
	}

	/**
	 * Modifie la taille du cercle centrale du terrain proportionnellement à la
	 * taille du terrain. PS : son diamètre est égale à 1/6 de la hauteur du
	 * terrain
	 */
	public void setCenterCircle() {
		...
		this.center.setFrameFromCenter(centerX, centerY, centerX + radius,
				centerY + radius);
	}
}
